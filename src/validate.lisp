...
(defun instance-table (source)
  (cond
    ...
    ((xml-document-p source)
     (normalize-instance (elem-to-value (document-root-element source))))
    ((and (stringp source) (plusp (length source)) (char= (char source 0) #\<))
     (instance-table (xml-protocol:decode source)))
...
(defun unwrap-xml-root (source validator)
  ...
  (cond
    ((xml-element-p source)
     (values (elem-to-value source) (xml-local-name source)))
    ((xml-document-p source)
     (unwrap-xml-root (document-root-element source) validator))
    ((and (stringp source) (plusp (length source)) (char= (char source 0) #\<))
     (unwrap-xml-root (xml-protocol:decode source) validator))
...
(defun check-facets (ctx restriction value)
  ...
  (let ((str (as-string value))
        ...
        (mini (xml-child restriction "minInclusive"))
        (maxi (xml-child restriction "maxInclusive"))
        (pat (xml-child restriction "pattern"))
        (tz (xml-child restriction "explicitTimezone"))
        (enums (xml-children-named restriction "enumeration"))
        (assertions (xml-children-named restriction "assertion")))
    ...
    (when (and mini num)
      (let ((bound (as-number (xml-attr mini "value"))))
        (when (and bound (< num bound))
          (vfail ctx (format nil "minInclusive ~A" bound) value))))
    (when (and maxi num)
      (let ((bound (as-number (xml-attr maxi "value"))))
        (when (and bound (> num bound))
          (vfail ctx (format nil "maxInclusive ~A" bound) value))))
    (when (and pat str (not (check-pattern (xml-attr pat "value") str)))
      (vfail ctx (format nil "pattern ~S" (xml-attr pat "value")) value))
    (when enums
      (unless (some (lambda (e)
                      (let ((ev (xml-attr e "value")))
                        (or (equal ev value)
                            (and str (string-equal ev str)))))
                    enums)
        (vfail ctx "enumeration" value)))
    (when (and tz str)
      (check-explicit-timezone ctx (xml-attr tz "value") str))
    (dolist (a assertions)
      (unless (xpath-true-p (xml-attr a "test") value)
        (vfail ctx (format nil "assertion ~S" (xml-attr a "test")) value)))))
...
(defun check-simple (ctx elem value)
  (let ((restriction (restriction-of elem))
    ...
    (cond
      ...
      (t
       (when restriction
         (let ((base (xml-attr restriction "base")))
...
(defun check-element (ctx elem value)
  (multiple-value-bind (min max) (parse-occurs elem)
    (let ((nillable (nillable-p elem))
          ...
          (inline (or (xml-child elem "simpleType") (xml-child elem "complexType"))))
      (labels ((one (v)
                   ...
                   (cond
                     ...
                     (alts
                      (let ((alt (find-if (lambda (a)
                                            (xpath-true-p (xml-attr a "test") v))
                                          alts)))
                        (if alt
                            (check-type-ref ctx (xml-attr alt "type") v)
...
(defun check-complex (ctx node value)
  (let ((*vctx* ctx)
    ...
    (when (and alts (object-p value))
      (let ((alt (find-if (lambda (a) (xpath-true-p (xml-attr a "test") value)) alts)))
        (if alt
            (return-from check-complex
              (check-type-ref ctx (xml-attr alt "type") value))
            (return-from check-complex
...
(defun check-tagged (ctx node disc value)
  (declare (ignore node))
  (let* ((prop (xml-attr disc "propertyName"))
    ...
    (let ((mapped (find-if (lambda (m)
                             (string-equal (xml-attr m "value") tag))
                           (xml-children-named disc "mapping"))))
      (unless mapped
        ...
        (let ((n (xml-attr c "name")))
          (cond
            ((xml-named-p c "defaultOpenContent")
             (setf default-open c))
            ((and (xml-named-p c "complexType") n)
             (setf (gethash n types) c))
            ((and (xml-named-p c "simpleType") n)
             (setf (gethash n types) c))
            ((and (xml-named-p c "element") n)
...
(defun root-type-node (validator)
  (let ((el (validator-root-element validator)))
    (or (and el (lookup-named validator (xml-attr el "type")))
        (and el (or (xml-child el "complexType") (xml-child el "simpleType")))
    ...
    (multiple-value-bind (payload root-name) (unwrap-xml-root instance validator)
      (when root-name
        (let ((expected (and (validator-root-element validator)
                             (xml-attr (validator-root-element validator) "name"))))
...
(defun decode-validating (source schema &key)
  "Well-formed decode then validate-instance. Returns xml-document.
   Signals xml-parse-error or xsd-schema-validation-error."
  (let ((doc (if (xml-document-p source)
                 source
                 (xml-protocol:decode source))))
    (validate-instance schema doc)
    doc))
